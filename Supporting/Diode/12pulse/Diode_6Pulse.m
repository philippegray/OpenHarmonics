%will adjust the input and output later?  I have to adjust at the beginning
%though...i can't do this later.
%cd('C:\Users\Phil\Dropbox\Masters\Matlab\Diode');
clear all;

%Changing after the meeting, new alpha etc.
global f Mag_abc Idc Pdc La Lb Lc Ts w phase Vdc L R Lout Rout lenHarm
...
h = 17;    %number of harmonics to evaluate
Ra = 2.0e-2; %thevenin equivalent resistance of phase a line in ohms
Rb = 2.0e-2; %thevenin equivalent resistance of phase b line in ohms
Rc = 2.0e-2; %thevenin equivalent resistance of phase c line in ohms
R = Ra;
La = 2e-1; %thevenin equivalent inductance of phase a line in ohms
Lb = 2e-1; %thevenin equivalent inductance of phase b line in ohms
Lc = 2e-1; %thevenin equivalent inductance of phase c line in ohms
L = La;
Rout = 0.02;%2e-2;
%Rdc = 0.01;
Lout = 0.5; %at 100 or even 10 for that matter, the current just keeps rising unbounded it seems.
Vbase = 1;
tolerance = 0.0001;
tolerance2 = 0.0001;
maxCount = 300;
...
Vout = 1.3; %assuming first that Vout is actually just a DC value...it won't be necessarily though so I will change this later.  This problem is directly analogous with the Idc FCM issues encountered in the VSC model.
...
Pload =9.21*Vout;%758*1.1;%0.8;%12; in units of Watts.
...
%Vsalphabeta*****************************************************************
Vin_pos = zeros(h,1); %at least now I will only allow positive sequence harmonic components
Vin_neg = zeros(h,1);
Vdc_vect_pos = zeros(h,1);

%V(2*lenHarm+2*h) = 0.05;
Vin_pos(1) = posSeq;
%Vin_neg(1) = negSeq;
Vin_pos(3) = 0.02+1i*0.02;
%Vin_neg(5) = 0.02+1i*0.02;
Vin_pos(7) = 0.01+1i*0.02;
%Vdc_vect_pos(1) = 0.5;
Vdc_vect_dc = Vout;%0.5;
%Vdc_vect_pos(2) = 0.1+1i*0.1;
%Vdc_vect_pos(3) = 0.1;
%Vin_pos(3) = 0.1;
%Vdc_vect!*****************************************************************
%**************************************************************************
%positive sequence comps
count = 1;
for m = 1:2:h*2
    Zi(lenHarm+1+m,1) = real(Vin_pos(count));
    Zi(lenHarm+1+m+1,1) = imag(Vin_pos(count));%-1i*Vin(count); %remember to change back to -1i*Vin(count)!
    count = count+1;
end
%negative sequence comps
count = 1;
for m = h*2:-2:1
    Zi(m-1,1) = real(Vin_neg(count));
    Zi(m,1) = imag(Vin_neg(count));%-1i*Vin(count); %remember to change back to -1i*Vin(count)!
    count = count+1;
end
%dc comps
...
count = 1;
for m = 1:2:h*2
    Zi(2*lenHarm+2+m,1) = real(Vdc_vect_pos(count));
    Zi(2*lenHarm+3+m,1) = imag(Vdc_vect_pos(count));
    count = count +1;
end
%**************************************************************************
...
mu = (pi/12)*ones(6,1);%(pi/3-0.2)*ones(6,1);
gamma = pi/3*ones(6,1);
...
while(true)
    ...
    for k = 1:7
        ...
        for i  = -h:1:h
            RotationMatrix(count:count+1,count:count+1,k) = ...
                [cos(i*(gamma_sum-phase)),-sin(i*(gamma_sum-phase));...
                sin(i*(gamma_sum-phase)),cos(i*(gamma_sum-phase))];
            count = count + 2;
        end
        for i = 0:h
            RotationMatrix(count:count+1,count:count+1,k) = ...
    ...
    for j = 1:7
        count = 1;
        for i  = -h:1:h
            derivRotation(count:count+1,count:count+1,j) = [-i*sin(i*(gamma_sum-phase)),...
                -i*cos(i*(gamma_sum-phase));i*cos(i*(gamma_sum-phase)) -i*sin(i*(gamma_sum-phase))];
            count = count + 2;
        end
        for i  = 0:1:h
            derivRotation(count:count+1,count:count+1,j) = [-i*sin(i*(gamma_sum-phase)),...
    ...
    for i = 1:7 %need to use Ts b/c I am solving for 7 intervals > num of intervals/period
        ...
        M(i+6) = 0 - ((L*(2*R+Rout)/(2*L+Lout)-R)*Idc_temp+...
    ...
    for i = 1:7
        for j = 1:6 
            ...
            J21(i,j) = calcJ21(Ap(1:end,1:end,Ts(i)),Np(1:end,1:end,Ts(i)),... %might be looking @wrong value??
                dMpmu(1:end,1:end,j,Ts(i)),Zi_arr(1:end,i),Mi_gamma);
            if j < i
                condMtx = derivRotation(1:end,1:end,i);
            else
                condMtx = zeros(3*lenHarm+1,3*lenHarm+1);
            ...
            J22(i,j) = calcJ22(Ap(1:end,1:end,Ts(i)),Np(1:end,1:end,Ts(i)),...
                ...
                condMtx,[Vabc_coeff(Ts(i),1:end),L/(2*L+Lout)]*constMtx);
        end
        condMtx = derivRotation(1:end,1:end,i);
        J22(i,7) = calcJ23(Ap(1:end,1:end,Ts(i)),Np(1:end,1:end,Ts(i)),...
            Zi,Mi_gamma,condMtx,[Vabc_coeff(Ts(i),1:end),L/(2*L+Lout)]*constMtx);
    end
...
%     for i = 1:6
%         if gamma(i) > pi/2 || gamma(i) < pi/6
%             gamma(i) = pi/3;
%         end
%         temp2 = rand(1)*pi/3;
% if mu(i) > gamma(i) || mu(i) < 0
% 	mu(i) = pi/6;%temp2;%rand(1)*gamma(i);%pi/6;
% end
%         if phase > pi/3
%             phase = 0;
%         elseif phase < -pi/3
