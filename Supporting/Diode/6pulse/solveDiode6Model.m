function [Ia Ib Ic fundCurrents phaseAng gamma mu Pload error] =...
    solveDiode6Model(Valphabeta,L,R,Rdc,Ldc,h,Vdc,Pdc,mu_i,gamma_i,phase_i)
...
while(true)
    ...
    %prev
    Vabc_coeff(2,1:end) = [1-L/(2*L+Lout),-1,L/(2*L+Lout)]*invCTFsmall; %confirmed dec. 13th
    ...
    Vabc_coeff(1,1:end) = [-L/(2*L+Lout),-1+L/(2*L+Lout),1]*invCTFsmall;%confirmed dec. 13th
    %Vabc_coeff(1,1:end) = [-L/(2*L+Lout),-1+L/(2*L+Lout),1]*invCTFsmall; 
%     Vabc_coeff(1,1:end) = [1-L/(2*L+Lout),-1,L/(2*L+Lout)]*invCTFsmall; %confirmed dec. 13th
%     Vabc_coeff(2,1:end) = [1,-L/(2*L+Lout),-1+L/(2*L+Lout)]*invCTFsmall;
%     Vabc_coeff(3,1:end) = [L/(2*L+Lout),1-L/(2*L+Lout),-1]*invCTFsmall;
%     Vabc_coeff(4,1:end) = [-1+L/(2*L+Lout),1,-L/(2*L+Lout)]*invCTFsmall;
%     Vabc_coeff(5,1:end) = [-1,L/(2*L+Lout),1-L/(2*L+Lout)]*invCTFsmall;
%     Vabc_coeff(6,1:end) = [-L/(2*L+Lout),-1+L/(2*L+Lout),1]*invCTFsmall;%confirmed dec. 13th
    ...
    for i = 1:7
        for j = 1:6 
            ...
            J22(i,j) = calcJ22(Ap(1:end,1:end,Ts(i)),Np(1:end,1:end,Ts(i)),...
                ...
                condMtx,[Vabc_coeff(Ts(i),1:end),L/(2*L+Lout)]*constMtx,L,R,Ldc,Rdc);
        end
        condMtx = derivRotation(1:end,1:end,i);
        J22(i,7) = calcJ23(Ap(1:end,1:end,Ts(i)),Np(1:end,1:end,Ts(i)),...
            Zi,condMtx,[Vabc_coeff(Ts(i),1:end),L/(2*L+Lout)]*constMtx,L,R,Ldc,Rdc);
    end
...
%     end
    ...
    %might be a way to strategically reduce mu's across the board while
    %compensating through modifying phaseAng 
%     sum_mu = 0;
%     for i = 1:6 
%        sum_mu = mu(i)+sum_mu; 
%     end
...
%     end
    ...
    if max(mu(1:6)) > pi/2 || min(mu(1:6)) < 0
        flag_error = flag_error +1;
        if max(mu(1:6)) > pi || min(mu(1:6)) < -pi/3
            flag_error = 2;
        end
    else
        flag_error = 0;
