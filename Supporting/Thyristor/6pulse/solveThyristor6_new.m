function [Ia,Ib,Ic,fundCurrents,alpha,mu_out] = solveThyristor6(Valphabeta,L,R,...
    Rdc,Ldc,h,Vdc,P)
...
Vllmax = sqrt(2)*sqrt(3)*abs(Valphabeta(lenHarm+2)+1i*Valphabeta(lenHarm+3));
if Vdc > Vllmax
    [Ia Ib Ic fundCurrents] = getI_abc_fundCurrents(zeros(3*lenHarm+1,1),h);
else
    ...
    mu = (pi/3-0.2)*ones(7,1); %Initialization
    %mu = 0.7*ones(7,1);
    mu(7) = alpha;
