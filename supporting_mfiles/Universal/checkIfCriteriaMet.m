...
for i = 1:2*numHarmonics+1
    for j = 1:length(MonitorList)/3
        if Vcurr(i,j) >= 5 %need this if statement for the even harmonics that are close to 0 but can vary from 10^-13 to 10^-12 for example each iteration which obviously doesn't matter which value...we don't care and it's a junk value.
